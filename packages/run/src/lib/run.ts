import { DisposeFunction, Drivers } from '@ogod/core';

export function run<D extends Drivers<SO, SI>, SO, SI extends Record<keyof SO, any>>
...
export function run<D extends Drivers<SO, SI>, SO, SI extends Record<keyof SO, any>>
    ...
    if (typeof drivers !== `object` || drivers === null) {
        throw new Error(
            `Argument given to run must be an object with driver functions as properties.`
        );
    }
    if (Object.keys(drivers).length === 0) {
        throw new Error(
            `Argument given to run must be an object with at least one driver function declared as a property.`
        );
    }
    const sources: any = {};
    const promises: any = {};
