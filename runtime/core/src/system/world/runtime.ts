...
import { OgodStateWorld } from "./state";
import { from } from "rxjs";
import { map } from "rxjs/operators";
import { OgodRuntimeSystemDefault } from '../runtime';
import { OgodStateInWorld } from '../../instance/in-world/state';

declare var self: OgodRuntimeEngine;

export class OgodRuntimeWorld extends OgodRuntimeSystemDefault {

    update(delta: number, state: OgodStateWorld) {
        const fullState = self.store.getState();
        const follow = state.follow ? fullState.instance[state.follow] as OgodStateInWorld : null;
        if (follow && state.camera && state.camera.offset) {
            ...
            from(state.entities).pipe(
                map((id) => fullState.instance[id] as OgodStateInWorld),
            ).subscribe((instance) => {
                instance.x = camera.x + instance.worldX - camera.worldX;
                instance.y = camera.y + camera.height - (instance.worldY - camera.worldY);
            });
        }
    }
}